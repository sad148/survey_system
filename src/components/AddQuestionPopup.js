...
class AddQuestionPopup extends Component {
    ...
    createOptions = (type) => {
        ...
        if(noOfOptions > 0 && noOfOptions < 8) {
            ...
            for(let i = 1;i <= noOfOptions; i++) {
                if(type == "radio")
                    options.push(<div style = {{marginBottom:"5px"}}><Radio value={i} disabled><Input id = {i} placeholder = { "Enter option " + i } onBlur = {() => this.updateQuestionObject(i, type)} autoComplete = "off" style = {{width:"50%"}}/></Radio></div>)
                else
                    options.push(<div style = {{marginBottom:"5px"}}><Checkbox  value={i} disabled><Input id = {i} placeholder = { "Enter option " + i } onBlur = {() => this.updateQuestionObject(i, type)} autoComplete = "off" style = {{width:"50%"}}/></Checkbox ></div>)
            }

            type == "radio" ? this.setState({options:(<RadioGroup>{options}</RadioGroup>)}) : this.setState({options:(<CheckboxGroup>{options}</CheckboxGroup>)})
        }
    ...
    updateQuestionObject = (id, type) => {
        ...
        this.state.questionAnsMap.options[id] = document.getElementById(id).value
    }
    ...
    render = () => {
        ...
        return (
            <div>
                <SkyLight hideOnOverlayClicked ref={ref => this.simpleDialog = ref} title="New Question" dialogStyles = {dialogCss} beforeOpen={this.executeBeforeModalOpen}
                >
                        ...
                        >
                            {getFieldDecorator('answerType', {
                                ...
                                <Select defaultValue = 'Select...' style = {{ width: 200 }} onChange={this.handleChange}>
                                    ...
                                    <Option value = "checkbox"><input type = "checkbox" disabled />Select multiple options</Option>
                                </Select>
                            )}
                        </FormItem>
